...
$install_list.ForEach({
    ...
    if (Test-Path -Path "$scripts_path/$($_.Filename).ps1" -PathType Leaf) {
        # Include file
        Write-Output -ForegroundColor Yellow "Exécution du script $($_.Name) : $scripts_path/$($_.Filename).ps1"
        Invoke-Expression -Command "$scripts_path/$($_.Filename).ps1"
        Write-Output -ForegroundColor Yellow "Fin de l'éxécution du script"
    }
})